...
def on_register_click():
    ...
    register_script_path = os.path.join(current_dir, 'register.py')
...
def on_login_click():
    ...
    if username and password:
        if login(username, password):
            ...
            # Construct the relative path to the register.py script
            register_script_path = os.path.join(current_dir, 'theshoekeeper.py')
            ...
            messagebox.showwarning("Error", "Login failed. Please check your username and password.")
    else:
...
tk.Label(window, text="USERNAME", font=("Comic Sans MS", 10)).place(x=350, y=150)
tk.Label(window, text="PASSWORD", font=("Comic Sans MS", 10)).place(x=350, y=200)

entry_username = tk.Entry(window, font=("Comic Sans MS", 10))
entry_username.place(x=440, y=150)

entry_password = tk.Entry(window, show="*", font=("Comic Sans MS", 10))
entry_password.place(x=440, y=200)

tk.Button( text="Log in", bg="#e6e6fa", command=on_login_click, compound="top", height=1, width=8).place(x=440, y=250)
tk.Button( text="Register", bg="#e6e6fa", command=on_register_click, compound="top", height=1, width=8).place(x=520, y=250)
